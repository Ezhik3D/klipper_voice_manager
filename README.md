...
Эти модули будут работать **только если встроите в макросы** простые строчки `M118` (ниже будет разбор). В остальном в комментариях всё указано: простое вкл/выкл.

---

## Ответы M118, которые будут отловлены для обработки

Пример: `M118 G28 START` будет отловлено и обработано как «парковка»,  
`M118 G28 X END` будет озвучено как «парковка оси X завершена».
...
Это главные шаблоны, какие `response` считывать на основе `M118`. Менять можно то, что **в кавычках**, и соответственно встраивать в макросы.
...
Если кто-то захочет озвучивать свои события по `M118` — например, обрыв прутка и т.п.
...
### Ответы M118 для CustomNotifier
...
Важно: так как у всех разное железо, система при первом запуске **применит профиль на основе вашего железа** и создаст файл-контейнер, из которого при дальнейших перезагрузках будет загружать данные в память. Время первого запуска зависит от железа.
...
Установка толшько зависимостей:
